...
namespace NopSolutions.NopCommerce.BusinessLogic.Content.NewsManagement
{
    ...
    public partial class NewsService : INewsService
    {
        ...
        /// <param name="count">News count to return</param>
        /// <returns>News item collection</returns>
        public List<News> GetAllNews(int languageId, int count, DateTime fromDate)
        {
            bool showHidden = NopContext.Current.IsAdmin;
            return GetAllNews(languageId, showHidden, count, fromDate);
        }
        ...
        /// <param name="count">News count to return</param>
        /// <returns>News item collection</returns>
        public List<News> GetAllNews(int languageId, bool showHidden, int count, DateTime fromDate)
        {
            return GetAllNews(languageId, showHidden, 0, count, fromDate);
        }
        ...
        /// <returns>News item collection</returns>
        public PagedList<News> GetAllNews(int languageId, bool showHidden,
            int pageIndex, int pageSize, DateTime fromDate)
        {
            if (pageSize <= 0)
            {
                pageSize = 10;
            ...
            var query = from n in _context.News
                        where (showHidden || n.Published) &&
                        (languageId == 0 || languageId == n.LanguageId)
                        && n.CreatedOn < fromDate
                        orderby n.CreatedOn descending
                        select n;
            var news = new PagedList<News>(query, pageIndex, pageSize);
            if (news.Count == 0)
            {
                news = GetAllNews(languageId, showHidden, pageIndex, pageSize);
            }
            return news;
        }
        ...
        /// <summary>
        /// Gets or sets a value indicating news count displayed on the main page on Left or Right column
        /// </summary>
        public int MainPageNewsCountOnLeftOrRightColumn 
        {
            get
            {
                int mainPageNewsCount = IoC.Resolve<ISettingManager>().GetSettingValueInteger("Display.MainPageNewsCountOnLeftOrRigthColumn");
                return mainPageNewsCount;
            }
            set
            {
                IoC.Resolve<ISettingManager>().SetParam("Display.MainPageNewsCountOnLeftOrRigthColumn", value.ToString());
            }
        }
