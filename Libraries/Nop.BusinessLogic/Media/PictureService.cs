...
namespace NopSolutions.NopCommerce.BusinessLogic.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        /// <param name="targetSize">The target picture size (longest side)</param>
        /// <param name="showDefaultPicture">A value indicating whether the default picture is shown</param>
        /// <param name="pictureName">The name of picture, it will display on first of first name</param>
        /// <returns></returns>
        public string GetPictureUrl(int imageId, int targetSize,
            bool showDefaultPicture, string pictureName)
        {
            var picture = GetPictureById(imageId);
            return GetPictureUrl(picture, targetSize, showDefaultPicture, pictureName);
        }
        ...
        /// <returns></returns>
        public string GetPictureUrl(Picture picture, int targetSize,
            bool showDefaultPicture, string pictureFileName)
        {
            string url = string.Empty;
            if (picture == null || picture.LoadPictureBinary().Length == 0)
            {
            ...
            lock (s_lock)
            {
                if (targetSize == 0)
                {
                    localFilename = string.Format("{0}.{1}", pictureFileName +"-"+ picture.PictureId.ToString(), lastPart);
                    if (!File.Exists(Path.Combine(this.LocalThumbImagePath, localFilename)))
                    {
                        if (!System.IO.Directory.Exists(this.LocalThumbImagePath))
                        {
                ...
                {
                    localFilename = string.Format("{0}_{1}.{2}", pictureFileName + "-" + picture.PictureId.ToString(), targetSize, lastPart);
                    if (!File.Exists(Path.Combine(this.LocalThumbImagePath, localFilename)))
                    {
                        if (!System.IO.Directory.Exists(this.LocalThumbImagePath))
                        {
