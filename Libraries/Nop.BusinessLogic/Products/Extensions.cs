...
namespace NopSolutions.NopCommerce.BusinessLogic.Products
{
    ...
    public static class Extensions
    {
        ...
        public static Product FindProductTag(this ICollection<Product> source,
            int productId, int tagId)
        {
            foreach (Product product in source)
                if (product.ProductId == productId)
                { 
                    foreach(ProductTag pt in product.NpProductTags)
                    {
                        if (pt.ProductTagId == tagId) return product;
                    }
                }
