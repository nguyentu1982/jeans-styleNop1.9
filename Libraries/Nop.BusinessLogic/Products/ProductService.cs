...
namespace NopSolutions.NopCommerce.BusinessLogic.Products
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        public List<Product> GetAllProductsDisplayedOnHomePage()
        {
            ...
            if (this.SortProductByCreatedDate)
            {
                 products = GetAllProductsDisplayedOnHomePageSortByCreatedDate();
            }
            else
            {
                var query = (from p in _context.Products
                            ...
                            !p.Deleted &&
                            p.ShowOnHomePage
                            select p).Take(this.NumberOfProductOnHomePage);
        ...
        /// <summary>
        /// Gets all products displayed on the home page sort by Created Date
        /// </summary>
        /// <returns>Product collection</returns>
        public List<Product> GetAllProductsDisplayedOnHomePageSortByCreatedDate()
        {
            bool showHidden = NopContext.Current.IsAdmin;
            ...
            var query = from p in _context.Products 
                        ...
                        !p.Deleted &&
                        p.ShowOnHomePage
                        orderby c.CategoryId, p.CreatedOn descending
                        select p;
            List<Product> products = query.ToList<Product>(); 
            if(products.Count >= this.NumberOfProductOnHomePage)
            {
                return products.Take(this.NumberOfProductOnHomePage).ToList<Product>();
            }
        ...
        /// <returns></returns>
        public List<Product> GetProductsOthersById(int productId)
        {
            int totalRecords = 0;
            var products = new List<Product>(); //Todo: TBD
            return products;
        }
        ...
        #region UpdateSizeToFilterInCategoryPage

        public void UpdateSizeToFilterInCategoryPage()
        {
            _context.CommandTimeout = 5000;
            _context.ProductSpecificationFilterSize();
            _context.CommandTimeout = null;
        }

        #endregion UpdateSizeToFilterInCategoryPage
        ...
        public int RecentlyAddedProductsNumberOnHomepage
        {
            get
            {
                int recentlyAddedProductsNumber = IoC.Resolve<ISettingManager>().GetSettingValueInteger("Display.RecentlyAddedProductsNumberOnHomepage", 6);
                return recentlyAddedProductsNumber;
            }
            set
            {
                IoC.Resolve<ISettingManager>().SetParam("Display.RecentlyAddedProductsNumberOnHomepage", value.ToString());
            }
        }
        ...
        /// <summary>
        /// Gets or sets a value indicating whether product is sorted by created date
        /// </summary>
        public bool SortProductByCreatedDate 
        {
            get
            {
                return IoC.Resolve<ISettingManager>().GetSettingValueBoolean("Product.SortedByCreatedDate");
            }
            set
            {
                IoC.Resolve<ISettingManager>().SetParam("Product.SortedByCreatedDate", value.ToString());
            }
        }

        /// <summary>
        /// Gets or sets a value indicating number of product return on homepage
        /// </summary>
        public int NumberOfProductOnHomePage
        {
            get
            {
                return IoC.Resolve<ISettingManager>().GetSettingValueInteger("Product.NumberOfProductOnHomePage");
            }
            set
            {
                IoC.Resolve<ISettingManager>().SetParam("Product.NumberOfProductOnHomePage", value.ToString());
            }
        }
        #endregion
    }
