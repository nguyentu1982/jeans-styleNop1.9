...
namespace NopSolutions.NopCommerce.BusinessLogic.Manufacturers
{
    ...
    public partial class ManufacturerService : IManufacturerService
    {
        ...
        /// </summary>
        /// <returns>Manufacturer collection</returns>
        public List<Manufacturer> GetAllManufacturers(int cateId)
        {
            bool showHidden = NopContext.Current.IsAdmin;
            return GetAllManufacturers(showHidden, cateId);
        }
        ...
        public List<Manufacturer> GetAllManufacturersByCateId(bool showHidden, int cateId)
        {
            showHidden = NopContext.Current.IsAdmin;
            return GetAllManufacturers(showHidden, cateId);
        }


        public List<Manufacturer> GetAllManufacturers(bool showHidden, int cateId)
        {
            string key = string.Format(MANUFACTURERS_ALL_KEY, showHidden);
            object obj2 = _cacheManager.Get(key);
            if (this.ManufacturersCacheEnabled && (obj2 != null))
            ...
            var query = from  m in _context.Manufacturers
                        join productManufacture in _context.ProductManufacturers
                        on m.ManufacturerId equals productManufacture.ManufacturerId
                        join productCategory in _context.ProductCategories
                        on productManufacture.ProductId equals productCategory.ProductId
                        orderby m.DisplayOrder
                        where (showHidden || m.Published) && 
                        (productCategory.CategoryId == cateId) &&
                        !m.Deleted                        
                        select m;
            var manufacturers = query.Distinct().ToList();

            //List<Manufacturer> result = new List<Manufacturer>();
            //foreach (Manufacturer manufacture in manufacturers)
            //{
            //    foreach (ProductManufacturer pm in manufacture.ProductManufacturers)
            //    {
            //        foreach (Categories.ProductCategory pc in pm.Product.ProductCategories)
            //        {
            //            if (pc.CategoryId == cateId)
            //            {
            ...
            //manufacturers = result.Distinct().ToList();
